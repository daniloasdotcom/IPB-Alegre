...
import 'package:app_igreja/itens.especiais.alternativa.dart';
...
class _HomePageAlternativeState extends State<HomePageAlternative> {
  List<NotificacaoModel> recado = <NotificacaoModel>[];
  final Uri _url =
  ...
  _getFeedbackFromSheet() async {
    ...
    jsonRecado.forEach((element) {
      ...
      NotificacaoModel notificacaoModel = NotificacaoModel(
           recado: '', hora: '' as int, notificacao: '');
      notificacaoModel.hora = element['hora'];
      notificacaoModel.notificacao = element['notificacao'];
      recado.add(notificacaoModel);
    });
    ...
    var time = Time(recado[0].hora, 0, 0); // Defina o horário desejado para a notificação diária
